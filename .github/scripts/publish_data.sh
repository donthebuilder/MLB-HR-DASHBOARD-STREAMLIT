...
# Per-player detail files (spray chart, pitch-type profile, pitcher arsenal).
# ~36 MB across ~400 small files. The app fetches ONE of these (~82 KB) only
# when someone opens a player, so this never affects normal page load -- and
# because the data branch is a single force-pushed orphan commit, it doesn't
# accumulate either.
if [ -d public/data/current/detail ]; then
  cp -r public/data/current/detail "$STAGE/public/data/current/"
fi
...
if git rev-parse --verify origin/data >/dev/null 2>&1; then
  ...
  # The detail/ directory is only regenerated by the slate workflows. Without
  # this, a grading run -- which doesn't build it -- would publish a tree with
  # no detail/ and silently break every player detail view until the next
  # slate run. The file loop above skips directories, so it's copied here.
  if [ -d /tmp/data-prev/public/data/current/detail ] && [ ! -d "$STAGE/public/data/current/detail" ]; then
    cp -r /tmp/data-prev/public/data/current/detail "$STAGE/public/data/current/"
  fi
fi
